			...
			<div class="module-dialog clear module-dialog-address module-dialog-show">
				<div class="dialog-panel">
						<div class="topbar">
								...
								<span class="dialog-close" @click="closehanle">x</span>
														...
														<div class="address-form">
																<div class="module-form-row">
																		<div class="form-item-v3">
																				<input type="text" class="js-verify" placeholder="收货人姓名" v-model="genggaiitem.name">
																				<div class="verify-error"></div>
																		</div>
																</div>
																<div class="module-form-row">
																		<div class="form-item-v3">
																				<input type="text" class="js-verify" placeholder="手机号" v-model="genggaiitem.phone">
																				<div class="verify-error"></div>
																		</div>
																</div>
																...
																<div class="module-form-row clear">
																		<div class="form-item-v3 select-item province-wrapper">
																				<select name="province_code" class="province select-province js-form-province js-verify"
																				 v-model="genggaiitem.province">
																						<option value="请选择省份">请选择省份</option>
																						<option :value="province.name" v-for="(province,index) in addList" :key='index'>
																...
																<div class="module-form-row clear">
																		<div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
																				<select class="city select-city js-form-city js-verify" v-model="genggaiitem.city">
																						<option value="请选择市">请选择市</option>
																						<option :value="city.name" v-for="(city,index) in cityList" :key='index'>{{city.name}}</option>
																				</select>
																		</div>
																		<div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
																				<select class="city select-city js-form-city js-verify" v-model="genggaiitem.county">
																						<option value="请选择区县">请选择区县</option>
																						<option :value="county" v-for="(county,index) in countyList" :key='index'>
																							{{county}}
																						</option>
																				</select>
																		</div>
																...
																<div class="module-form-row">
																		<div class="form-item-v3">
																				<input type="text" class="js-verify" placeholder="详细地址，如街道名称，楼层，门牌号码等"
																				 v-model="genggaiitem.add">
																				<div class="verify-error"></div>
																		</div>
																</div>
																<div class="module-form-row fn-clear">
																		<input type="checkbox" v-model="genggaiitem.checked">
																		<span class="blue-checkbox"></span>设为默认
																</div>
																<div class="dialog-blue-btn big-main-btn js-verify-address" @click="submitadd">
																		<a>保存</a>
																</div>
														</div>
												</div>
...
<script>
	import addList from '../lib/newaddList.js'
	export default {
		data(){
			return {
				addList,
		...
		methods:{
			...
			submitadd(){
				this.$http.post('/api/cart/updataaddress',this.genggaiitem)
				.then(res=>{
		...
		computed:{
			cityList(){
				...
				this.addList.forEach(item=>{
					if(item.name==this.genggaiitem.province){
						citylist=item.city
					}
				})
				return citylist
			...
			countyList(){
				...
				this.cityList.forEach(item=>{
					if(item.name==this.genggaiitem.city){
						countylist=item.districtAndCounty
					}
				})
				return countylist 
			}
