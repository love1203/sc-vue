						...
						<div class="box-inner js-checkout-address-panel ">
							<div class="address-common-table js-multiple-address-panel">
								<ul class="address-item-list clear js-address-item-list">
									<li class="js-choose-address" :class="{'selected-address-item':item.checked}"  v-for="(item,index) in addressdata" :key='index' @click="xuanzhongadd(item)">
										<div class="address-item">
											...
											<div class="bianji">
												<span class="genggai1" @click.prevent.stop="updataaddress(item)">更改</span>
												<span class="genggai2" @click.prevent.stop="removeaddress(item._id)">删除</span>
											</div>
										</div>
									</li>
									<li class="add-address-item js-add-address" @click="addaddress">
										<p>使用新地址</p>
									</li>
								</ul>
							</div>
				...
				<div class="gray-box">
					...
					<div class="box-inner invoice-box js-invoice-box"> 
						...
						<div class="invoice-detail"> 发票抬头： 
							<div class="radio-box"> 
								<label @click="fapian(1)"> 
									<input type="radio" class="hide"> 
									<span class="blue-radio" :class="{'blue-radio-on':fapianbool==1}"><a></a></span>  个人
								</label> 
								<label @click="fapian(2)"> 
									<input type="radio" class="hide"> 
									<span class="blue-radio" :class="{'blue-radio-on':fapianbool==2}"><a></a></span>  单位
								</label> 
							</div> 
							<div class="module-form-row form-item fn-hide js-invoice-title" v-show="fapianbool==2"> 
								<div class="module-form-item-wrapper no-icon small-item"> 
									<input type="text" class="js-verify" v-model="gstaitou" placeholder="请填写公司发票抬头"> 
								</div> 
							</div> 
						</div> 
				...
				<div class="gray-box">
					...
					<div class="box-inner ui-goods-cart">
						...
						<div class="cart-table">
							<div class="cart-group js-cart-group">
								...
								<div class="cart-items" v-for="(item,index) in checkoutcartdata" :key='index'>
									<div class="items-thumb">  
										<a href="javascript:;" target="_blank"><img :src="item.productImage"></a>  
									</div>
									...
									<div class="subtotal">
										<div class="subtotal-cell">  ¥ {{xiaoji(item)}}  </div> 
									</div>
					...
					<div class="box-inner"> 
						<div class="last-payment clear"> 
							<span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click="submitdingdan"> <a>提交订单</a> </span> <span class="prices fn-right">应付金额： 
							<em>¥ {{checkedprice+yunfei}}</em></span> 
						</div> 
					</div>
				</div>
			</div>
			<addresspop  v-show="addshow" @closetk='closetk1'></addresspop>
			<gengai v-show="genggaishow" @closeggit='closeggitt' :genggaiitem='genggaiitem'></gengai>
			<!-- <tiankuan v-show="addshow" @closetk='closetk1'></tiankuan> -->
		</div>
</template>

<script>
	import tiankuan from '../components/tiankuan.vue'
	import gengai from '../components/gengai.vue'
	...
	export default {
		...
		computed:{
			...
			checkoutcartdata(){
				...
				this.cartdata.forEach(item=>{
					if(item.checked){
						checkoutcartdata.push(item)
					}
				})
				return checkoutcartdata
			},
			checkedprice(){
		...
		methods:{
			...
			removeaddress(id){
				this.$http.post('/api/cart/removeaddress',{id:id})
				.then(res=>{
			...
			updataaddress(item){
				...
				this.genggaiitem=item
			},
			closeggitt(){
				this.genggaishow=false
				this.getcartall()
			},
			fapian(num){
				this.fapianbool=num
			...
			xuanzhongadd(item){
				this.$http.post('/api/cart/checkaddress',{item})
				.then(res=>{
					//console.log(res.data)
					this.getcartall()
				})
			...
			submitdingdan(){
				this.addressdata.forEach(item=>{
					...
					let fapiaoinfo=''
					if(this.fapianbool==1){
						fapiaoinfo='个人'
					}else{
						fapiaoinfo=this.gstaitou
					}
					let ndate=new Date()
					let nyear=ndate.getFullYear()
					let nmounth=ndate.getMonth()+1
					if(nmounth<10){
						nmounth='0'+nmounth
					}
					...
					let dateinfo=nyear+'-'+nmounth+'-'+nday
					let dingdaninfo=Date.now()
					let zjinfo=this.checkedprice+this.yunfei
					
					this.$http.post('/api/cart/order',{dingdaninfo,dateinfo,dizhiinfo,fapiaoinfo,zjinfo,checkoutcartdata:this.checkoutcartdata})
					.then(res=>{
						this.$http.delete('/api/cart/removecheckeddata')
						.then(res1=>{
							this.getcartall()
							this.$router.push({path:'/payment',query:{orderId:dingdaninfo}})
						})
...
.bianji{width: 100%;height: 20px;position: absolute;bottom: 10px;right: 10px;}
.bianji .genggai1{float: right;display: block;width: 50px;height: 20px;font-size: 14px;color: #fff;background: #6A8FE5;line-height: 20px;text-align: center;border-radius: 3px;}
.bianji .genggai2{float: right;display: block;width: 50px;height: 20px;font-size: 14px;color: #fff;background: red;line-height: 20px;margin-right: 10px;text-align: center;
border-radius: 3px;}
</style>
