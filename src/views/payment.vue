	    	...
	    	<div class="content page-order-payment">
	    	    <div class="gray-box clear">
	    	        ...
	    	        <div class="box-inner payment-checkout-panel clear">
	    	            <span class="jianguo-blue-main-btn big-main-btn js-payment-order" v-if="!orderListdata.isPay" @click="ispayment(orderListdata.dingdaninfo)">
	    	                <a>现在支付</a>
	    	            </span>
										<span class="jianguo-blue-main-btn big-main-btn js-payment-order" v-if="orderListdata.isPay">
												<a>已支付</a>
										</span>
	    	            <span class="prices"> 应付金额：   <em><span>¥ </span>{{orderListdata.zjinfo}}</em>   </span>
	    	        </div>
	    	    </div>
	    	    <div class="confirm-detail">
	    	        <div class="info-title">订单编号</div>
	    	        <p class="info-detail">{{orderListdata.dingdaninfo}}</p>
	    	    </div>
	    	    <div class="confirm-detail">
	    	        <div class="info-title">收货信息</div>
	    	        <p class="info-detail" v-if="orderListdata.dizhiinfo">姓名：{{orderListdata.dizhiinfo.name}}</p>
	    	        <p class="info-detail" v-if="orderListdata.dizhiinfo">联系电话：{{orderListdata.dizhiinfo.phone}}</p>  <p class="info-detail" v-if="orderListdata.dizhiinfo">详细地址：{{orderListdata.dizhiinfo.province}} {{orderListdata.dizhiinfo.city}} {{orderListdata.dizhiinfo.county}} {{orderListdata.dizhiinfo.add}}</p>
	    	    </div>
	    	    <div class="confirm-detail">
	    	        <div class="info-title">发票信息</div>
	    	        <p class="info-detail">发票类型：电子发票</p>
	    	        <p class="info-detail">发票抬头：{{orderListdata.fapiaoinfo}}</p>
	    	    ...
	    	    <div class="confirm-goods-table">
	    	        <div class="cart-items clear" v-for="(item,index) in orderListdata.checkoutcartdata" :key='index'>
	    	            <div class="name fn-left hide-row">
	    	                <div class="name-cell"> 
	    	                    <a href="javescript:;">{{item.productName}}</a><br>
	    	                </div>
	    	            </div>
	    	            <div class="subtotal fn-right">
	    	                <div class="subtotal-cell">{{xiaoji(item)}}</div>
	    	            </div>
	    	            <div class="goods-num fn-right">{{item.productNum}}</div>
	    	            <div class="price fn-right">{{item.salePrice}}</div> 
	    	        </div>
	    	    ...
	    	    <div class="order-discount-line">
	    	        <p> 商品总计： <span>¥ {{orderListdata.zjinfo}}</span> </p>
	    	    </div>
	    	</div>
	    </div>
</template>
...
<script>
	export default{
		...
		computed:{
			...
			orderListdata(){
				...
				this.orderdata.forEach(item=>{
					if(item.dingdaninfo==this.$route.query.orderId){
						orderListdata=item
					}
				})
		...
		methods:{
			...
			ispayment(dingdaninfo){
				this.$http.post('/api/cart/ispay',{dingdaninfo})
				.then(res=>{
					alert('支付成功')
