		...
		<div class="loginbox">
			...
			<form @submit.prevent="submit">
				<div class="inputgroup">
					<label>用户名</label>
					<input type="text" placeholder="请输入用户名" v-model="newuser.name">
				</div>
				<p class="errtext" v-if="errors.name">{{errors.name}}</p>
				<div class="inputgroup">
					<label>邮箱</label>
				...
				<div class="inputgroup">
					<label>确认密码</label>
					<input type="password" placeholder="请再次输入密码" v-model="newuser.password2">
				</div>
				<p class="errtext" v-if="errors.password2">{{errors.password2}}</p>
...
<script>
	export default {
		...
		methods:{
			submit(){
				this.$http.post('/api/users/register',this.newuser)
				.then(res=>{
